...
class RecipeDatabase:
    ...
    def ingredients_delete(self, recipeid, connection):
        ...
        total = connection.execute(res)
        for x in total:
            res2 = self.ingredients.delete().where(
                self.ingredients.c.id == x.ingredient_id)
            connection.execute(res2)
        y = self.ingredients_list.delete().where(
            self.ingredients_list.c.recipe_id == recipeid)
        connection.execute(y)
    ...
    def directions_delete(self, recipeid, connection):
        ...
        connection.execute(res)
        y = self.directions_list.delete().where(
            self.directions_list.c.recipe_id == recipeid)
        connection.execute(y)
    ...
    def ingredient_insert(self, recipeid, ingredient, conn):
        ...
        ins = self.ingredients_list.insert().values(
            recipe_id=one_row.id,
            ingredient_id=ingredientpkey[0],
            quantity=ingredient['amount'])
        res = conn.execute(ins)

    # insert a direction into a recipe given the recipe id
    # and a direction
    # and a number for the direction number
    ...
    def recipe_insert(self, name, author, country, course,
                      ingreds, directions, image, connection):
        idx = 1
        ...
        ins = self.recipes.insert().values(name=name, country=country,
                                           course=course, image=image,
                                           views=int(0), user_id=idx)
